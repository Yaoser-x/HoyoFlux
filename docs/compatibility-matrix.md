...
| 启动参数 | 手工预引用后再次引用 | 配置档 `args` 数组转换为严格 Win32 参数编码 | 带空格、引号与反斜杠的参数保持独立边界 |
| 配置解析 | `stoi` 异常路径 | schema 2 严格字段、`from_chars` 与显式范围检查 | 错误配置报告字段和可用行列位置而非崩溃 |
| 注入面 | 手工 DLL loader、通用 shellcode | 固定页面 code stub，由各 PatchPlan 独立持有，不加载 DLL | 只保留最小必要机制 |

## 验证状态

- 1.1.0 本地发行准备：Debug 与 Release 构建及完整 CTest 均为 11/11 通过；发布资源、PE 元数据、依赖和 ZIP 内容审计通过。最终包的 EXE 诊断烟测在独立中文路径完成，报告写入软件目录且 AppData 未变化。该记录不代表 UAC 交互或真机游戏门已通过。
